...
/*
 * Initiating Partecipant
 */

angular.module('flowableModeler').controller('FlowableInitiatingPartecipantCtrl',
    ['$scope', '$modal', '$timeout', '$translate', function ($scope, $modal, $timeout, $translate) {

        // Config for the modal window
        var opts = {
            template: 'editor-app/configuration/properties/initiating-partecipant-popup.html?version=' + Date.now(),
            scope: $scope
        };
...
angular.module('flowableModeler').controller('FlowableInitiatingPartecipantPopupCtrl',
    ['$rootScope', '$scope', '$http', 'editorManager', function ($rootScope, $scope, $http, editorManager) {
        ...
        $scope.save = function() {
            ...
            if(selectedShapes){
                ...
                for (var i = 0; i < labels.length; i++){
                    if(labels[i].id.endsWith("init_part_name")){
        ...
        $scope.createNewProcess = function(){
            ...
            success(function(data) {
                ...
                $scope.property.value.id = data.id;
            }).
