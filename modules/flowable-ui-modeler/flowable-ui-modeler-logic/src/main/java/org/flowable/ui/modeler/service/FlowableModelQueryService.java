...
public class FlowableModelQueryService {
    ...
    public ResultListDataRepresentation getChoreographyModelsToIncludeInAppDefinition() {
        ...
        List<String> addedModelIds = new ArrayList<>();
        List<Model> models = modelRepository.findByModelType(AbstractModel.MODEL_TYPE_CHOREOGRAPHY, ModelSort.MODIFIED_DESC);
    ...
    public ModelRepresentation importChoreographyModel(HttpServletRequest request, MultipartFile file) {
        ...
        if (fileName != null && (fileName.endsWith(".bpmn") || fileName.endsWith(".bpmn20.xml"))) {
            try {
                ...
                model.setName(name);
                model.setDescription(description);
                model.setModelType(AbstractModel.MODEL_TYPE_CHOREOGRAPHY);
                Model newModel = modelService.createModel(model, modelNode.toString(), SecurityUtils.getCurrentUserObject());
                return new ModelRepresentation(newModel);
